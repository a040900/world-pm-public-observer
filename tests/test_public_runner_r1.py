...
class T(unittest.TestCase):
 def test_partial(self):
  r=maker.queue_shadow_classification(maker_bid=.48,queue_ahead=1000,maker_size=5,trades=[{"side":"SELL","price":.47,"size":.2,"sourceTimestampMs":2000}],queue_timeline=[],placed_at=1)
  self.assertEqual(r["definite"]["status"],"PARTIAL_SHADOW_FILL"); self.assertAlmostEqual(r["definite"]["fillShares"],.2)
 def test_cache(self):
